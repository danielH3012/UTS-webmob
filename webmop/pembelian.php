...
$stmt = $conn->query("SELECT id_pembelian FROM pembelian ORDER BY id_pembelian DESC LIMIT 1");
$latestiddep = $stmt->fetch_assoc();
$urut = 1;
if ($latestiddep) {
    $latestNumber = $latestiddep['id_pembelian'];
    $urut = $latestNumber + 1;
}
$newiddep = $urut;
                        ...
                        <table id="departemenTable" class="table table-bordered table-striped table-hover">    
                            <thead class="text-center">
                                <tr>
                                    ...
                                    <th class='text-center'>Manufaktur</th>
                            ...
                            <tbody>
                                ...
                                if ($result && $result->num_rows > 0) {
                                    ...
                                    while ($departemen = $result->fetch_assoc()) {
                                        ...
                                        echo "<td class='text-center'>" . htmlspecialchars($departemen['id_pembelian']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($departemen['id_barang']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($departemen['manufaktur']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($departemen['jumlah_pembelian']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($departemen['harga']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($departemen['total_pembelian']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($departemen['status']) . "</td>";
                                        echo "<td class='text-center'>";
                                        echo "<div class='d-flex justify-content-center'>";
                                        echo "<form action=\"status_pembelian.php\" method=\"post\"> 
                                                <button class='btn btn-warning btn-sm edit-btn mr-1' style=\"background-color: chocolate;\" name=\"id\" type=\"submit\" value=\"".$departemen['id_pembelian']."\">change status</button> 
                                              </form>";
                                        echo "<button class='btn btn-warning btn-sm edit-btn mr-1' data-bs-toggle='modal' data-bs-target='#editdepartemenModal' 
                                         data-id='" . htmlspecialchars($departemen['id_pembelian'])
                                         . "' data-idbar='" . htmlspecialchars($departemen['id_barang'])
                                         . "' data-manu='" . htmlspecialchars($departemen['manufaktur'])
                                         . "' data-jumlah='" . htmlspecialchars($departemen['jumlah_pembelian'])
                                         . "' data-harga='" . htmlspecialchars($departemen['harga'])
                                         . "' ><i class='fas fa-edit'></i> Edit</button>";
                                        echo "<button class='btn btn-danger btn-sm delete-btn' data-id='" . htmlspecialchars($departemen['id_pembelian']) . "'><i class='fas fa-trash'></i> Delete</button>";
                                        echo "</div>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="adddepartemenModalLabel">Masukan Pembelian</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="add_pembelian.php" method="post">
                    <div class="mb-3">
                        <label for="iddep" class="form-label">Kode Pembelian</label>
                        <input type="text" class="form-control" id="iddep" name="idpem" value="<?php echo htmlspecialchars($newiddep); ?>" readonly>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="iddep" class="form-label">Manufaktur</label>
                        <input type="text" class="form-control" id="iddep" name="manufaktur" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="iddep" class="form-label">Harga</label>
                        <input type="text" class="form-control" id="iddep" name="harga" required>
                    </div>
                    <div class="mb-3">
...
<div class="modal fade" id="editdepartemenModal" tabindex="-1" aria-labelledby="editdepartemenModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editdepartemenModalLabel">Edit Pembelian</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="edit_pembelian.php" method="post">
                    <div class="mb-3">
                        <label for="edit_iddep" class="form-label">Kode Pembelian</label>
                        <input type="text" class="form-control" id="edit_idpem" name="idpem" readonly>
                    </div>
                    <div class="mb-3">
                        <label for="edit_idbar" class="form-label">ID Barang</label>
                        <br>
                        <select name="id_barang" id="edit_idbar" style="width:100%;"required>
                    <?php 
                        while ($pilihan1 = $barang1->fetch_row()) {
                            echo "<option value=". $pilihan1[0].">". $pilihan1[1]."</option>" ; 
                    }
                    ?> 
                    </select>
                    </div>
                    <div class="mb-3">
                        <label for="edit_manufaktur" class="form-label">Manufaktur</label>
                        <input type="text" class="form-control" id="edit_manufaktur" name="manufaktur" required>
                    </div>
                    <div class="mb-3">
